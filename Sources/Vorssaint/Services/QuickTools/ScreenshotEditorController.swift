...
final class ScreenshotEditorModel: ObservableObject, BackdropEditing {
    ...
    func continueDrag(to point: CGPoint, final: Bool = false, constrained: Bool = false) {
        ...
        let point = constrained && (tool == .arrow || tool == .line)
    ...
    func endDrag(at point: CGPoint, isTap: Bool, constrained: Bool = false) {
        if gestureCancelled { gestureCancelled = false; return }
        ...
        if !isTap { continueDrag(to: point, final: true, constrained: constrained) }
        if editingSelectedAnnotation {
